...
internal static class Debbie {
    ...
    private static readonly Dictionary<string, Action> _commands = new() {
        { "help", static () => ShowHelpWindow = !ShowHelpWindow },
        { "stats", static () => _showStatsMenu = !_showStatsMenu },
        { "clear", static () => Cheats.FillWorld(Engine.World, "air") },
        { "fill", static () => Cheats.FillWorld(Engine.World, Canvas.BrushMaterial) },
        { "nuke", static () => Cheats.NukeWorld(Engine.World) },
    };
    ...
    public static void ApplyCommand(string command = null) {
        ...
        if (int.TryParse(_commandInput, out int c)) {
            ...
            _commandInput = "";
        } else if (_commands.TryGetValue(_commandInput, out Action command_action)) {
            command_action.Invoke();
            ShowCommandPrompt = false;
            _lastCommand = _commandInput;
            _commandInput = "";
        } else {
    ...
    public static void Draw() {
        ...
        if (_showMaterialsMenu && ImGui.Begin("Material Inspector", ref _showMaterialsMenu, ImGuiWindowFlags.AlwaysAutoResize | ImGuiWindowFlags.NoCollapse)) {
            string CursorMat = Engine.World.InBounds(Engine.MousePos) ? Engine.World.Get(Engine.MousePos).ID : "air";
            ...
            foreach (System.Reflection.PropertyInfo Property in _loadedMaterial.GetType().GetProperties()[..^2]) {
                ImGui.TableNextRow();
            ...
            if (_loadedMaterial.Tags.Count > 0) {
                ...
                string TagsList = "";
                foreach (string Tag in _loadedMaterial.Tags) { TagsList += Tag + ", "; }
        ...
        if (_showStatsMenu && ImGui.Begin("Stats", ref _showStatsMenu, ImGuiWindowFlags.AlwaysAutoResize | ImGuiWindowFlags.NoCollapse)) {
            ...
            ImGui.Text($"Chunk: {(Engine.World.InBounds(Engine.MousePos) ? Engine.World.GetChunk(Engine.MousePos).Position : "None")}");
            ImGui.Text($"Material: {(Engine.World.InBounds(Engine.MousePos) ? Materials.GetName(Engine.World.Get(Engine.MousePos).ID) : "None")}");
            ImGui.Text($"Active Chunks: {Engine.World.ActiveChunks.Count}");
